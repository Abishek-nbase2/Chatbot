...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Initialize Groq API (will be configured when API key is provided)
        logger.info("Groq API will be initialized when API key is provided")
...
app = FastAPI(title="Chatbot API with Groq", version="1.0.0", lifespan=lifespan)
...
def setup_groq(api_key: str):
    """Setup Groq API with provided key"""
    global groq_client
    try:
        ...
        logger.error(f"Error configuring Groq API: {str(e)}")
        return False
...
@app.post("/configure-groq")
async def configure_groq(request: dict):
    """Configure Groq API key"""
    api_key = request.get("api_key")
    if not api_key:
        raise HTTPException(status_code=400, detail="API key is required")
    
    success = setup_groq(api_key)
    if success:
        return {"message": "Groq API configured successfully"}
    else:
        raise HTTPException(status_code=500, detail="Failed to configure Groq API")
...
async def chat(request: ChatRequest):
    ...
    try:
        if groq_client is None:
            raise HTTPException(status_code=400, detail="Groq API not configured. Please provide API key first.")
