...
export class RAGSystem {
    ...
    isInitialized = false;
    vectorIndex = new Map();
    constructor(ollamaService, geminiService) {
        this.ollama = ollamaService;
        this.gemini = geminiService;
    }
    async buildKnowledgeBase(pdfData) {
        try {
    ...
    async processPageContent(page) {
        ...
        if (page.text && page.text.trim()) {
            try {
                const pageChunks = await this.semanticChunkText(page.text, 600, 'tas2781.pdf', page.pageNumber);
    ...
    async searchSimilarChunks(query, topK = 15) {
    ...
    simpleTextSearch(query, topK = 15) {
        ...
        for (const chunk of this.documentChunks) {
            ...
            if (chunk.metadata.topic) {
                const topicMatches = queryWords.filter(word => chunk.metadata.topic.toLowerCase().includes(word)).length;
                score += topicMatches * 5; // Bonus for topic matches
            }
            // Key terms matching
            if (chunk.metadata.keyTerms) {
                const keyTermMatches = chunk.metadata.keyTerms.filter(term => queryWords.some(word => term.toLowerCase().includes(word) || word.includes(term.toLowerCase()))).length;
...
- Use key terms to enhance technical accuracy`;
    ...
    async semanticChunkText(text, maxChunkSize = 1000, source = '', page) {
        try {
    ...
    async identifySemanticSections(text) {
...
${text}`;
        try {
            const response = await this.gemini.generateResponse(prompt, 'You are a technical document analyzer. Respond only with valid JSON.');
    ...
    async splitLargeSection(section, maxSize) {
    ...
    extractKeyTermsFromText(text) {
        ...
        if (capitalizedWords) {
            terms.push(...capitalizedWords.filter(word => word.length > 3 && !['The', 'This', 'That', 'When', 'Where', 'What', 'How'].includes(word)));
        }
        return [...new Set(terms)].slice(0, 10); // Limit to 10 most relevant terms
    }
    inferSemanticType(text) {
    ...
    fallbackChunkText(text, chunkSize, source, page) {
        const chunks = [];
