...
const ApiKeySetup: React.FC<ApiKeySetupProps> = ({ onConfigured }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/configure-groq', {
        method: 'POST',
        headers: {
      ...
      setError('Failed to configure Groq API. Please check your API key.')
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <div className="max-w-md mx-auto mt-8 p-6 bg-white rounded-lg shadow-lg">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            Groq API is now ready. Redirecting to chat...
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="max-w-md mx-auto mt-8 p-6 bg-white rounded-lg shadow-lg">
      ...
      <p className="text-gray-600 mb-6 text-sm">
        To use the chatbot, you need to configure your Groq API key.
